 ...
 */
export function lineTruncation(rootElement, lines, lineHeight, options) {
  ellipsisCharacter = options.ellipsisString ? options.ellipsisString : ellipsisCharacter;
  ...
  if (Math.floor(rootElement.clientHeight / 2) > truncateHeight) {
    ...
    options.handler(appendElementNode(childNodes, rootElement, lines, lineHeight));
  } else {
    options.handler(truncateElementNode(rootElement, rootElement, lines, lineHeight));
  }
}
